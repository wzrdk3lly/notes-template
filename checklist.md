...
- 'L1` - can liquidation occur Before Defaults?
- `L2` - Are there any situations that can exist where the borrow cannot be liquidated?
- `L3` - Can debt be closed without repayment? ie, can the borrower pocket their loan without repaying.
- `L4` - repayments paused while liquidations enabled
- `L5` - token disallows or whitelist can cause existing positions to be liquidated without repayment
- `L6` Borrower Immediately Liquidated After Repayments Resume after being paused. This usually disadvantages a borrower if there is no grace period after pausing to allow for repayments.
- `L7` Liquidator takes collateral with insufficient repayment. In the example given if a borrower has multiple positions open, when they get liquidated the liqidator takes collateral from all of those positions by only needed to repay for the lowest positions loan.
- `L8` Infinite loan rollover
- `L9` repayments send to 0 address`
- `L10` borrower repayment partially credited. IF they have multiple positions open and they attempt to make a repayment, remaing left overs after payment should be applied to remaining positions and not just the first position in line.
